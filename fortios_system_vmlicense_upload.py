    ...
    system_vmlicense:
        description:
            - Possible parameters to go in the body for the request.
              Specify vmlicense source, filename
        default: null
        type: dict
        required: true
        suboptions:
...
- hosts: localhost
  vars:
   ...
   ssl_verify: "False"
  tasks:
  - name: Perform license upload locally.
    fortios_system_vmlicense_upload:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
      ...
      system_vmlicense:
        ...
        source: "upload"
    register: fortios_system_vmlicense_upload_result

  - debug:
      var:
        # please check the following status to confirm
        fortios_system_vmlicense_upload_result.meta.results.status
'''
...
def filter_system_vmlicense_data(json):
    option_list = ['file_content', 'filename', 'source']
    dictionary = {}
...
def system_vmlicense(data, fos, check_mode=False):
    vdom = data['vdom']

    system_vmlicense_data = data['system_vmlicense']

    filtered_data = {}
    filtered_data['source'] = system_vmlicense_data['source']
    if hasattr(system_vmlicense_data, 'format_partition'):
        filtered_data['format_partition'] = system_vmlicense_data['format_partition']
    if filtered_data['source'] == 'upload':
        try:
            filtered_data['file_content'] = base64.b64encode(open(system_vmlicense_data['filename'], 'rb').read()).decode('utf-8')
        except Exception:
            filtered_data['file_content'] = ''
    else:
        filtered_data['filename'] = system_vmlicense_data['filename']
